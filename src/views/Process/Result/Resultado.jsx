...
const Resultado = (props) => {
  ...
  useEffect(() => {
    async function setListaCumplimientos() {
      const auxResultado = await APIEvaluation.consultResult(props.eval);
      setCumplimientos(auxResultado.result);
      setFinalResult(auxResultado.finalresult);
      console.log(auxResultado, finalResult, cumplimientos);
    }
    ...
    <Grid container direction="column">
          ...
          <TableBody>
            {cumplimientos.map((cump) => (
              <TableRow key={cump.objetivo}>
                ...
                <TableCell width="10%" align="center">
                  {checkedOrig ? cump.nDeseadoOrig : cump.nDeseado}
                </TableCell>
                <TableCell width="10%" align="center">
                  {checkedOrig ? cump.nAlcanzadoOrig : cump.nAlcanzado}
                </TableCell>
                <TableCell width="10%" align="center">
                  {checkedOrig ? cump.porcDeseadoOrig : cump.porcDeseado}
                </TableCell>
                <TableCell width="10%" align="center">
                  {checkedOrig ? cump.porcAlcanzadoOrig : cump.porcAlcanzado}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Grid container item xs={12}>
        ...
        <Tooltip style={{ marginTop: "13px" }} title="Ver tabla">
          <IconButton aria-label="ver-tabla" onClick={handleModalOpen}>
            <Help />
          </IconButton>
        </Tooltip>
      </Grid>
      ...
      <Grid container item xs={12}>
        <Table className={classes.table} aria-label="simple table">
          <TableHead>
            <TableRow>
              <TableCell colSpan={2} width="20%" align="center">
                Nivel de cumplimiento organizacional
              </TableCell>
          ...
          <TableBody>
            {finalResult.map((result) => (
              <TableRow key={0}>
                <TableCell width="10%" align="center">
                  {checkedOrig ? result.PuntuacionOrig : result.Puntuacion}
                </TableCell>
                <TableCell width="10%">
                  {checkedOrig ? result.EtapaOrig : result.Etapa}
                </TableCell>
                <TableCell width="80%">
                  {checkedOrig ? result.DescriptionOrig : result.Description}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
