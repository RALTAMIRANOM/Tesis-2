...
const Cuestionario = (props) => {
      ...
      subcategorias: [
        {
          ...
          preguntas: [
            {
              texto:
                "Los objetivos de inversión en hardware, redes y telecomunicaciones " +
  ...
  const [subcategSelected, setSubcategSelected] = React.useState([
    -1,
    -1,
    -1,
    -1,
  ...
  const handleSubcategClick = (event, indexCateg, indexSubcateg) => {
    ...
    //oculta las preguntas el cuestionario
    else newSubcategSelected[indexCateg] = indexSubcateg; //muestra las preguntas del cuestionario
    setSubcategSelected(newSubcategSelected);
    let auxPreguntas = [
      ...categorias[indexCateg]["subcategorias"][indexSubcateg].preguntas,
    ];
  ...
  const actualizarSeleccion = async (indexCateg) => {
    let auxCateg = [...categorias];
    auxCateg[indexCateg]["subcategorias"][
      subcategSelected[indexCateg]
    ].preguntas = tablaPreguntas;
    setCategorias(auxCateg);
    const res = await APIEvaluation.registerQuestionary(auxCateg);
  ...
  const terminarEvaluacion = async () => {
    if (
      categorias.filter(
        (x) =>
          x.subcategorias.filter(
            (y) => y.preguntas.filter((z) => z.respuesta != -1).length != 0
          ).length != 0
      ).length != 0
    ) {
  ...
  return (
    <div className={classes.root}>
      <Grid item xs={12}>
          ...
          return (
            <Accordion>
              <AccordionSummary
                ...
                id="panel1a-header"
                style={{
                  backgroundColor: colors[indexCateg % 5].back,
                  color: colors[indexCateg % 5].text,
                  opacity: 0.6,
                }}
              >
                <Typography className={classes.heading}>
                  <strong>
                    {desafio.nombre} (
                    {
                      desafio.subcategorias.filter(
                        (x) =>
                          x.preguntas.filter((y) => y.respuesta != -1).length ==
                          x.preguntas.length
                      ).length
                    }
                    /{desafio.subcategorias.length})
                  </strong>
                </Typography>
              </AccordionSummary>
              <AccordionDetails>
                    ...
                    {desafio.subcategorias.map((subcateg, key) => {
                      return (
                        <ListItem
                          button
                          onClick={(event) =>
                            handleSubcategClick(event, indexCateg, key)
                          }
                        >
                          ...
                          <ListItemIcon
                            style={{
                              color:
                                subcateg.preguntas.length ==
                                subcateg.preguntas.filter(
                                  (y) => y.respuesta != -1
                                ).length
                                  ? "limegreen"
                                  : "black",
                ...
                <Grid item xs={7}>
                  {subcategSelected[indexCateg] == -1 && (
                    <div style={{ marginTop: "10px" }}>
                      Haga clic en algún cuestionario del lado izquierdo para
                      habilitar la lista de preguntas.
                    </div>
                  )}
                  {subcategSelected[indexCateg] > -1 && (
                    <div>
                      ...
                      >
                        <Button
                          variant="outlined"
                          color="primary"
                          onClick={() => cancelarSeleccion(indexCateg)}
                        >
                          Regresar
                        </Button>
                        <Button
        ...
        <Typography
          variant="h3"
          className={classes.txtContainerTitleCenter}
          fontWeight="fontWeightBold"
        >
          <Button
            variant="outlined"
